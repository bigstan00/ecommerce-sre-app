...
// any other import (including './server', './lib/redisClient', fastify,
// redis, etc). Auto-instrumentation works by monkey-patching modules (http,
// redis, ...) at `require()` time — if those modules get required before
// this file registers the instrumentations below, they never get patched
// and no spans are produced. Import ordering here is load-bearing, not
// stylistic.
//
...
// (http://localhost:4318, cart-service) live in .env.example. This service
// doesn't use dotenv elsewhere (its config already falls back to defaults
// when env vars are unset), so tracing follows the same convention: if
// OTEL_EXPORTER_OTLP_ENDPOINT/OTEL_SERVICE_NAME aren't set in the real
// environment, the OTel SDK's own spec-defined defaults apply.
...
const sdk = new NodeSDK({
  ...
  // Auto-instruments HTTP server/client (including this service's calls to
  // Catalog via CATALOG_SERVICE_URL) and its Redis client (`redis` v4) — no
  // manual span code. `fs` instrumentation is disabled because it's
  // extremely noisy (every file read/write becomes a span) and isn't one of
  // the integrations CONTRACTS.md calls out for this service.
  instrumentations: [
