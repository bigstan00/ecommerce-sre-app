...
// any other import (including './app', './config', './db/pool', fastify,
// pg, etc). Auto-instrumentation works by monkey-patching modules (http,
// pg, ...) at `require()` time — if those modules get required before this
// file registers the instrumentations below, they never get patched and no
// spans are produced. Import ordering here is load-bearing, not stylistic.
//
...
// (http://localhost:4318, auth-service) live in .env.example.
...
const sdk = new NodeSDK({
  ...
  // Auto-instruments HTTP server/client and this service's Postgres client
  // (`pg`) — no manual span code. `fs` instrumentation is disabled because
  // it's extremely noisy (every file read/write becomes a span) and isn't
  // one of the integrations CONTRACTS.md calls out for this service.
  instrumentations: [
